...
This skill provides reference knowledge — DB schema, daemon mechanics, file paths — for Claude Code sessions helping users with their polily install. The same content (minus this introduction) is loaded by polily's own internal AI agent at runtime; the single-source-of-truth lives in polily's `polily/agents/skill_sources/core/*.md`.
<!-- /external-only -->
