...
- **`events.event_metadata`** — Polymarket-curated `context_description` (the canonical "what's this event about" blurb) plus `context_updated_at`. **If `context_updated_at` is within the last 24h, read this first and let it be your baseline understanding** — it saves one broad-strokes WebSearch (~30s-1min per analysis). Only WebSearch for specific catalysts the description doesn't cover (confirming a quoted timeline, looking up a specific news item). If `context_updated_at` is older than 24h, the description may be stale — supplement with WebSearch. Polily's daemon keeps this field fresh via the `regen_stale_event_descriptions` step (v0.12.0) so when Polymarket flips their flag the description updates within a tick or two.
