...
- **Movement-triggered analyses inject `triggering_movements:` context into the agent prompt** (T-1, backlog re-scoped after investigation). When polily's daemon auto-dispatches an analysis because `_check_event_trigger` detected magnitude × quality crossing thresholds, the per-call ephemeral block now carries a `triggering_movements:` subsection listing every sub-market that moved in the last 60s, ordered by magnitude DESC. Each line shows `market_id`, `label`, yes-price delta, and M/Q scores. The agent no longer has to reverse-engineer the trigger story by querying `movement_log` itself — it sees the cross-market picture (same direction or split, magnitude distribution, label mix) directly in the prompt. Manual / scan / scheduled triggers are unaffected (no movement context applies). Single-file change in `polily/agents/narrative_writer.py` with documentation update in `skill_sources/core/07_per_call_inputs.md` (regenerated `manual.md`). 8 new tests cover positive injection, empty-state graceful absence, non-movement skip, window cutoff, NULL prev_yes_price rendering, and ephemeral-structure regression guard.

- **Every analysis prompt now carries `event_metadata_freshness:` block** (T-2, backlog re-scoped after evidence gathering). polily computes the age of Polymarket's curated `event_metadata.context_updated_at`, labels staleness (`fresh` <24h / `stale` 24-72h / `very_stale` >72h), exposes Polymarket's `context_requires_regen` flag value, and emits an imperative guidance string (e.g. very-stale → *"DO NOT use context_description as authoritative baseline. WebSearch is MANDATORY"*). Driven by `data/logs/agent_feedback.log` evidence on 2026-05-15: agent caught Polymarket leaving descriptions 5-6 days stale on events 108031 / 73106 / 51456 with `context_requires_regen=false`, so the flag-only regen flow never fired. Agent previously had to parse `context_updated_at` itself; now polily labels the staleness explicitly and the agent acts on the label. Applies to all trigger sources (not just movement). Companion doc fix: `skill_sources/core/04_data_freshness.md` moves `events.event_metadata` out of "Bucket 4 — Static" into a new "Bucket 5 — Externally-curated, freshness-tracked" (the prior classification contradicted the v0.12.0 regen flow). 10 new tests cover staleness tiers, graceful handling of missing / malformed / non-dict / no-timestamp metadata, polymarket_flag_set faithfulness, trigger-source agnosticism, and ephemeral-structure regression. Single-file impl in `polily/agents/narrative_writer.py`.
...
- **SKILL.md generator polish from Whis third-pass review.** Three minor cosmetic improvements: (1) §9 Source 1 now explicitly states "if active = `'official'`, this Source 1 path does not apply — proceed to Source 2" instead of leaving it implicit; (2) generator's `_filter_audience` now strips one preceding space from inline audience-tag blocks so mid-line strips (e.g. table cells like `text. <!-- internal-only -->note<!-- /...--> |`) don't leave a double-space artifact; (3) `_normalize_whitespace` post-processing step strips per-line trailing whitespace and collapses 3+ consecutive newlines to a single blank line — `_build_*_content` also rstrips the body before adding the single trailing newline so end-of-file is clean. Whis verdict: "Approve for ship."

- **SKILL.md §9 methodology lookup hardened to a 3-source fallback ladder** (Whis re-review). Initial design did a single `curl` from polily master, which would 404 in the pre-release window (default.md doesn't exist on master until v0.12.0 merge) and silently pass "404: Not Found" to Claude as methodology text — catastrophic. New ladder: (1) try user's polily.db for user_strategy.text with concrete acceptance criteria mirroring internal §8 fallback (non-empty, ≥5 lines, structural markdown, doesn't ask to execute, reads like methodology); (2) fall through to local install's `default.md` via `polily.__file__` + Read tool; (3) only as last resort, `curl --fail` GitHub master with explicit graceful failure ("I couldn't load polily's methodology; want me to retry or proceed from first principles?"). Plus: §4 dropped a dangling `official_strategy_path` / `§8` cross-reference that was leaking into SKILL.md after audience split. YAML description widened to cover chat-consultation triggers (follow-up questions, structure_score interpretation) on top of dev triggers, so users asking "为什么 polily 说 edge 很薄" actually activate the skill.
...
[Unreleased]: https://github.com/ShiyuCheng2018/polily/compare/v0.12.2...dev
[0.12.2]: https://github.com/ShiyuCheng2018/polily/releases/tag/v0.12.2
