...
async def test_main_screen_digit_6_opens_changelog(tmp_path):
    """`8` is bound to show_changelog and the action mounts ChangelogView.
    ...
    v0.12.0: digit 7 was reassigned to show_strategy when StrategyView
    was added between config and changelog; changelog moved from 7 → 8.
    Test name retained for git-blame continuity.

    Can't rely on `pilot.press('8')` because the default tasks view has
    ...
    async with app.run_test(size=(160, 40)) as pilot:
        ...
        # (a) Binding table routes "8" to show_changelog (v0.12.0+;
        #     was "7" pre-strategy page, "6" pre-v0.10.0).
        bindings = {b.key: b.action for b in screen.BINDINGS}
        assert bindings.get("8") == "show_changelog", (
            f"digit 8 should trigger show_changelog; bindings: {bindings}"
        )
