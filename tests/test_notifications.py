...
def test_add_and_get_unread(polily_db):
    add_notification(polily_db, title="[WATCH] BTC 68000",
...
def test_mark_read(polily_db):
    add_notification(polily_db, title="Test", body="Body")
    unread = get_unread_notifications(polily_db)
    assert len(unread) == 1
    mark_read(polily_db, unread[0]["id"])
    assert len(get_unread_notifications(polily_db)) == 0


def test_mark_all_read(polily_db):
    add_notification(polily_db, title="T1", body="B1")
    add_notification(polily_db, title="T2", body="B2")
    add_notification(polily_db, title="T3", body="B3")
    assert len(get_unread_notifications(polily_db)) == 3
    mark_all_read(polily_db)
    assert len(get_unread_notifications(polily_db)) == 0


def test_unread_ordered_newest_first(polily_db):
    add_notification(polily_db, title="First", body="B1")
    add_notification(polily_db, title="Second", body="B2")
    unread = get_unread_notifications(polily_db)
